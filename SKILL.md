...
- **deliver-gate**（/goal 退出 A③ 后，接 verification patch §C）：low 自动 / high 强制人审（§8.5）；high-risk codex 必复核（R9，patch §C.1），codex 不可用降级 advisory 升级人；**阻塞 re-entry 只限 high-risk**（抽样命中 low 只创建待清审查项，不阻塞当前 deliver，§8.7 E）；**high-risk 或抽样命中强制出可视化页面（schema §8b，复用两层结构）**；同 spec-gate 拆 ready/approved：可视化页面生成 = ready，人点通过 = approved
...
| spec-gate 唯一硬审（rubric 五项 §8.5）；spec-gate 可视化两层（Layer 1 白话 + Layer 2 具体）dispatch 独立 subagent + ready/approved 拆分（completed 只认 approved） | §8.7 B |
