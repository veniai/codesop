...
1. **dispatch 全新独立 subagent**（codesop spec-gate 的，非主 AI，也独立于 brainstorming 阶段的 evidence-pack subagent）读 spec + evidence pack，渲染两层：
   - **Layer 1 白话摘要**（3 秒入口，不提术语）——4 块：要解决的问题 / 实际会改变什么 / 为什么这样改 / 明确不改变什么（≤5 行、不用文件名/章节号/术语、**不抄 spec 原文**、subagent 重写）
...
- **deliver-gate**（/goal 退出 A③ 后，接 verification patch §C）：low 自动 / high 强制人审（§8.5）；high-risk codex 必复核（R9，patch §C.1），codex 不可用降级 advisory 升级人；**阻塞 re-entry 只限 high-risk + 抽样命中（§8.7 E）**，普通 low-risk 自动过不阻塞；**high-risk + 抽样命中强制出可视化页面（schema §8b，复用两层结构）**；同 spec-gate 拆 ready/approved：可视化页面生成 = ready，人点通过 = approved
