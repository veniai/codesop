...
### Changed
- README 中英文全面重设计：AI 安装提示（结构化多步指令）、痛点开场、核心亮点展示、详情折叠化
- 副标题从技术术语罗列改为"让 AI 拥有 SOP 纪律"
...
### Added
- Skill patch 机制：`patches/superpowers/` 目录存放修改后的 skill 文件，setup 自动同步
  - writing-plans：删除 Execution Handoff（pipeline re-entry 处理下一步）
  - finishing-a-development-branch：跳过 4 选项菜单，直接 push + PR
- worktree 条件化：从 ★（必走）改为仅用户明确要求时插入，默认只开分支
...
### Changed
- 工作台摘要从 7 字段精简为 2 固定 + 1 条件：**状态**（合并阶段+进度）、**分支**、**注意**（仅异常时显示）
- 删除长期目标（在 PRD 中，不重复）、文档状态/阻塞/最近决策合并为条件字段（无内容不输出）
- §4.5 Complete Example 摘要从 7 行缩减为 3 行
