...
- 任务完成写审计：`$XDG_STATE_HOME/codesop/audit.jsonl`（回退 `~/.local/state/codesop/audit.jsonl`）一行，字段 `intent/risk/profile/floor_reason/evidence/approver/ts`；可选默认开，关闭不影响执行——观察工具非 gate（不拦 Agent，只供回溯）
- 不确定 → 先调用 /codesop
