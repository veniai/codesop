## codesop 路由卡 (v5)（可忽略——熟悉项目可跳过；opt-out: `touch ~/.claude/.codesop-router-disabled`）

新任务必须先判 profile（见下），再按 profile 组装链路；`minimal` 任务可不输出任务对齐块（理解+阶段+Skill）。
完整 pipeline 定义见 /codesop。

### Profile 分档（v5：流程按风险比例，floor 不可降）
...
| Profile | 条件 | 行为 |
|---------|------|------|
| `minimal` | 低风险、低歧义、局部、可回滚、**不含**鉴权/迁移/部署/公共接口 | 直接执行 + diff/测试验证；不强制对齐块、spec-gate、HTML、人审 |
| `standard` | 多步骤、跨文件、ambiguity=high | 轻量目标 + 必要时 plan + 阶段末独立验证 |
| `governed` | 鉴权、数据迁移、公共接口、部署、破坏性、外部影响 | 正式 spec + 人审 + 独立复核 + 机械证据 + 回滚 |

**floor 不可降**：Agent 只能升档或请求人裁决，不能低于判定的 profile；缺风险信息默认升档（不默认 minimal）。单文件鉴权/迁移即使 1 文件也 governed。governed 人审是用户风控（不可逆操作），模型识别后请求，批准权在用户——不是 codesop 拦 Agent，是用户对自己不可逆操作的确认。
...
- 任务完成写审计：调 `write_audit()`（`lib/profile.sh`：`source ~/codesop/lib/profile.sh; write_audit <intent> <risk> <ambiguity> <blast> <override> <reversible> <declared_profile> <evidence> <approver>`）写一行 `$XDG_STATE_HOME/codesop/audit.jsonl`（回退 `~/.local/state/codesop/audit.jsonl`）；字段 intent/risk/ambiguity/blast/override/reversible/declared_profile/floor/floor_reason/evidence/approver/ts/violation（write_audit 内部调 judge_profile 算 floor，非法参拒写+非零退出）；观察工具非 gate（不拦 Agent，只供回溯）
- 不确定 → 先调用 /codesop
