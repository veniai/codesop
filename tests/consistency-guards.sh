...
# templates/init 已删（v4.4.2 删孤儿 prompt.md），README/CLAUDE/PRD 架构段不该再列
# 只查架构树格式（├──/└── init/），不查历史叙事（§2.4/§3 记录提 templates/init/prompt.md）
if grep -qE "(├──|└──) init/" README.md CLAUDE.md PRD.md; then
  fail "F: README/CLAUDE/PRD 架构段仍列已删的 init/"
fi
