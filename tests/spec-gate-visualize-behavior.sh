...
echo "=== §8.7 B: dispatch 独立 subagent（交叉检验）—— 术语锚 ==="
assert_in_file "$SKILL" "dispatch 全新独立 subagent" "dispatch 独立 subagent（合并：独立于 brainstorming）"
echo "  PASS dispatch 独立 subagent"

echo "=== §8.7 B: spec 实质为主 + evidence pack 为辅（保 spec 实质 + rubric）==="
...
echo "=== §8.7 B: ready/approved 拆分（URL=ready，人点通过=approved）—— 术语锚 ==="
...
echo "=== schema §8: spec 实质为主 + spec-gate 可视化（合并）==="
assert_in_file "$SCHEMA" "spec 实质为主" "schema §8 spec 实质为主 + spec-gate 可视化"
echo "  PASS schema §8 spec 实质为主"

echo "=== schema §8: Layer 1 白话摘要（4 块，保留 1 代表）==="
assert_in_file "$SCHEMA" "要解决的问题" "Layer 1 代表块（要解决的问题）"
echo "  PASS Layer 1 白话摘要"
