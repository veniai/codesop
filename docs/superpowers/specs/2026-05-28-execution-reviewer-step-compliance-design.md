...
elif [ ! -f "$sdd_spec" ]; then
  echo "  ⚠ subagent-driven-development spec-reviewer-prompt.md not found in plugin, skipping patch"
fi
...
if [ -f "$sdd_cq" ] && [ -f "$sdd_cq_patch" ]; then
  ...
  echo "  ⚠ subagent-driven-development code-quality-reviewer-prompt.md not found in plugin, skipping patch"
fi
```
...
**已知限制**：控制器的 re-dispatch 循环没有 retry cap——如果 spec reviewer 反复给 ❌（实现者修不好），循环会无限进行。这是控制器层面的问题，需要单独修 subagent-driven-development SKILL.md，不在本 spec 范围内。
...
**追加项**：Codex PR #29 另有一条意见（re-dispatch 耗尽后缺 blocking），该问题属于控制器层面而非 reviewer prompt，本 spec 在 §4 "已知限制" 中显式标注，留待单独修复。
