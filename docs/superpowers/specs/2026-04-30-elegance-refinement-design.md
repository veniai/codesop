...
精简理由：`source: codesop-pipeline` 是 Pipeline Re-entry 过滤任务的锚点（SKILL.md §3 re-entry step 2 用它过滤），不可删。metadata 格式是可测合同，保留。删掉的是 ☆/★ 的解释性文本、衔接任务 subject 模板等 AI 可推断的内容。
...
Codex Rev：加回"完成后 TaskUpdate(completed)"——与 task hygiene / re-entry 直接相关，不是 AI 一定能推断的合同行为。
...
- 保留 3 条硬合同：固定 4 section、不得加额外 heading/divider、末行必须是最后一行
- 删除其余格式禁令
- 压缩示例：只保留一个 complete example（合并 proposing/continuing），删除重复
...
合并：
3. **查表层**（变更影响矩阵）→ 压缩为 5 类高风险项（命令/脚本、环境变量/配置、依赖、API/接口、目录/架构），保留弱模型兜底
4. **checklist 层**（自检清单）→ 删除。5 条 checklist 是矩阵的子集
...
从 ~40 行压缩到 ~18 行。删掉完整 11 行矩阵（AI 用语义层判断即可），保留 5 类高风险映射作为兜底提示。删掉 5 条 checklist（矩阵的子集）。
...
精简 prompt 前先把可测合同固化到测试里，确保删 prompt 不丢合同。

| 合同 | 当前载体 | 精简后载体 |
|------|----------|-----------|
| metadata.source = codesop-pipeline | SKILL.md | SKILL.md（保留） |
| task subject 不含 ☆/★ | SKILL.md | SKILL.md（保留） |
| 输出固定 4 section | SKILL.md | SKILL.md（保留 3 条硬合同） |
| 文档判定输出格式 | AGENTS.md | AGENTS.md（保留） |
| AGENTS.md 关键字符串 | AGENTS.md | `tests/detect-environment.sh`（需验证） |

无需新增测试——现有 9 个测试套件已覆盖合同一致性。精简后跑 `tests/run_all.sh` 回归即可。
...
- **精简过度**：Codex Rev 已修正——保留 metadata 合同、保留 3 条硬合同、保留 5 类高风险映射，不过度删
- **格式回归**：保留 3 条硬合同（固定 4 section、不得加额外 heading/divider、末行必须是最后一行）+ 1 个 complete example 作为格式锚点
- **弱模型退化**：保留的 5 类高风险映射是 Codex 建议的弱模型兜底，比删光矩阵更稳
